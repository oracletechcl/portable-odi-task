...
Move `Web` into the explicitly selected target architecture. This strategy is bound to the same target profile used for `to-be.md` and should be regenerated if the target architecture changes.
...
flowchart TD
    ...
    assess["Assess Web Contracts and Process Flows"]
